...
int SDLIMG_isJPG(SDL_RWops *src)
{
	int start;
	int is_JPG;
	int in_scan;
...
SDL_Surface *SDLIMG_LoadJPG_RW(SDL_RWops *src)
{
	int start;
	...
	//if ( !SDLIMG_Init(SDLIMG_INIT_JPG) ) {
	//	return NULL;
	//}
	...
	if(setjmp(jerr.escape)) {
		...
		SDL_RWseek(src, start, RW_SEEK_SET);
		SDLIMG_SetError("JPEG loading error");
		return NULL;
	}
...
int SDLIMG_InitJPG()
{
	SDLIMG_SetError("JPEG images are not supported");
	return(-1);
}
